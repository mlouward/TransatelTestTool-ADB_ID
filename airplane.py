...
def airplane(index, duration):
    """ Deletes the APN with the selected ID for the selected phone.

    Args:
        index: The index of the phone.
        apn_id: The id of the APN to delete.        
    """
    num = tuple(number_to_imsi.items())[int(index) - 1][0]
    id = imsi_to_id[number_to_imsi[num]]
    subprocess.run(["airplane.bat", id, duration, num])

if __name__ == "__main__":
    number_to_imsi, imsi_to_id = get_dictionaries()
    airplane(sys.argv[1], sys.argv[2])
