...
adb -s %1 shell input keyevent KEYCODE_WAKEUP & adb -s %1 shell input swipe 100 500 100 0 & adb -s %1 shell input keyevent KEYCODE_MENU & adb -s %1 shell svc data enable & adb -s %1 shell svc wifi disable & timeout 1 > nul && echo [!date!-!time:~0,8!] Data test initiated. (PHONE: %3, URL: %2, NB: %4) >>..\logs\datalog.txt
set /a "nb=%4"
