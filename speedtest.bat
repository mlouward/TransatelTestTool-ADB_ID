...
adb -s %1 shell input keyevent KEYCODE_WAKEUP & adb -s %1 shell input swipe 100 1000 100 0 & adb -s %1 shell input keyevent KEYCODE_MENU & adb -s %1 shell svc data enable & adb -s %1 shell svc wifi disable & adb -s %1 shell su -c "settings put global multi_sim_data_call %4 && am broadcast -a android.intent.action.SUB_DEFAULT_CHANGED"
